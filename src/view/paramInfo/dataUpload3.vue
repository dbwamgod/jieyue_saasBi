  		...
  		<el-row :gutter="10" type="flex" align="middle" class='toolbar-box' style=" margin-left:0px; margin-right:0px;">
              <el-col :span="24" >
                    ...
                    <span>导入</span>
  							...
  							<a :href="srcUrl" style="margin-right:10px;" class='fr' download="全国目标阈值导入模板.xls">
  								<span class="resend-btn resend-btn-start"  style="padding: 3.5px 17px;">
      ...
      <el-row >
        <el-col :span="24">
          <el-table ref="multipleTable" v-loading="loading" border :data="tableData" stripe >
            ...
            <el-table-column align="center" prop="orgId" label="机构编码" ></el-table-column>
            <el-table-column align="center" prop="orgNm" label="机构名称" ></el-table-column>
            <el-table-column align="center" prop="dayId" label="会计日期	" ></el-table-column>
            <el-table-column align="center" prop="saleAmt" label="目标值" ></el-table-column>
            <el-table-column align="center" prop="saleQty" label="目标值类型 " ></el-table-column>
  ...
  export default {
    data() {
      return {
        search:'',
        drop:'',
        importFileUrl:api.getGoalsImprot(),
        srcUrl:require('../../../static/file/qgtarget.xls'),
        dropVisible:false,
        value1:dateFun.getDateYMD(0),
        fileList:[],
        tableData: [],
        options: constValue.valueList.target,
        selectValue:'',
        shows:true,
        currentPage:1,
	      pageSize: 10,
    ...
    methods:{
    	...
    	fetchData(){
	    		var vm = this;
		    	this.$http.post(api.getGoalsList(),
		    		{
	    ...
	    exportBtn(){
       	...
       	if(this.$common.isempty(vm.value1,'导出日期不能为空！','warning',vm)){
       			window.open(api.getTaskExport(vm.value1,3),"_blank");
       	}
      },
